...
async function renditionCallback(source, rendition, params) {
    ...
    const uriBase = `${endpoint}vision/v3.0/ocr?language=${requestedLang}&detectOrientation=true`;
    ...
    try {
        ...
        jsonResponse = JSON.stringify(jsonResponse, null, '  '); // necessary to keep this line so the unit tests pass
        console.log('response json', jsonResponse);
        fs.writeFileSync(rendition.path, jsonResponse);
        return(jsonResponse);
    } catch (e) {
        throw new GenericError('The Azure OCR API call returned an error: ' + e.message);
    }
}
