...
exports.main = worker(async (source, rendition, params) => {
    ...
    const url = `${endpoint}vision/v3.0/analyze?visualFeatures=Categories,Description,Color&details=&language=${language}`;
    const options = {
        method:"POST",
        body: JSON.stringify({
            url: source.url
    ...
    if (jsonResponse.code) {
        ...
        throw new GenericError(`The Azure Analyze Image Api failed with ${jsonResponse.code}: ${JSON.stringify(jsonResponse.message)}`);
    }
    if (jsonResponse.error) {
        throw new GenericError(`The Azure Analyze Image Api failed with: ${JSON.stringify(jsonResponse.error)}`);
    }
